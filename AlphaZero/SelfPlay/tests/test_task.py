...
class TestAlphaZero(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_game.get_next_state = MagicMock()
        self.mock_game.get_encoded_state = lambda state: np.zeros((3, 3))
    ...
    def test_self_play_memory_structure(self):
        ...
        for state, probs, outcome in memory:
            ...
            self.assertEqual(state.shape, (3, 3), msg="Check if state has shape (3,3)")
