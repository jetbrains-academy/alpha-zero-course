...
4. In `get_game_ended` function identify the end of the game. Return `1` if `player` is win, `-1` if `-player` is win, `0` if the game is not finished yet and `1e-4` if the game ended in a draw.
