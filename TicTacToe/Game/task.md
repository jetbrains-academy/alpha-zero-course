...
2. In `get_game_ended` function identify the end of the game. Return `1` if `player` is win, `-1` if `-player` is win, `0` if the game is not finished yet and `1e-4` if the game ended in a draw.
3. In `change_perspective` function invert the board. For `player` board should stay the same, but for `-player` 1 and -1 should be flipped. 
