...
class TestConnectFour(unittest.TestCase):
    ...
    def test_valid_moves(self):
        ...
        valid_moves = self.game.get_board().get_valid_moves()
    ...
    def test_game_end_conditions(self):
        ...
        for _ in range(4):
            self.game.get_board().execute_move(1, 0)
    ...
    def test_change_perspective(self):
        ...
        self.game.get_board().execute_move(1, 0)  # Player 1 move
        changed_board = self.game.change_perspective(self.game.get_board(), -1)
        self.assertEqual(changed_board._pieces[5, 0], -1)  # Ensure the perspective is inverted

    def test_get_encoded_state(self):
        """Test encoding the board state"""
        self.game.get_board().execute_move(1, 0)  # Player 1 move
        encoded_state = self.game.get_board().get_encoded_state()
        # print(encoded_state)
